...
DW_SCHEMA = "dw"
RE_SELECTOR = re.compile(r"^\+?[A-Z0-9_]+\+?$", re.IGNORECASE)
...
def make_graph(models: ModelCollection) -> nx.DiGraph:
    graph = nx.DiGraph()
...
def make_subgraph(graph: nx.DiGraph, select: list[str]) -> nx.DiGraph:
    """
...
def make_intermediate_relation(table: str, database: str | None = None) -> ClickHouseRelation:
    return ClickHouseRelation(table=f"{table}__tmp", database=database)


def execute_statement(
    session: Session, statement: str, parameters: dict[str, Any] | None = None, log: bool = False
) -> CursorResult[Any]:
    if log:
        log.debug(statement)
    ...
    if log:
        log.debug(f"Done in {timing.elapsed_seconds_formatted}")
    return result


def run_model(adapter: ClickHouseAdapter, model: ModelType, use_alembic: bool = True) -> str | None:
            ...
            if use_alembic:
                ...
                with adapter.create_session() as session:
                    if not adapter.has_table(
                        ...
                        execute_statement(session, statement, log=True)

        elif materialization == Materialization.CREATE_REPLACE:
            with adapter.create_session() as session:
                if use_alembic:
                    intermediate_relation = make_intermediate_relation(
                        table=target_relation.table, database=target_relation.database
                    )
...
def get_names_from_select(select: list[str] | None) -> list[str] | None:
    if select is None:
        return select
    else:
...
# def format_name(model: ModelType, select_names: list[str] | None) -> str:
...
def format_relation(relation: ClickHouseRelation) -> str:
...
def format_type(model: ModelType) -> str:
    if issubclass(model, BaseTable):
        return "table"
    elif issubclass(model, BaseView):
        return "view"
...
class Graph(BaseModel):
    ...
    def validate_select(cls, value: list[str] | None) -> list[str] | None:
        ...
        for selector in value:
            ...
            if not RE_SELECTOR.fullmatch(selector):
                raise ValueError(
                    f"Invalid selector '{selector}'. Expected formats: Model, +Model, Model+, +Model+"
                )
    ...
    def graph(self) -> nx.DiGraph:
        if self._graph is None:
            ...
            graph = make_graph(models)
            if self.select:
                graph = make_subgraph(graph, self.select)
            self._graph = graph
        ...
        if self._models is None:
            self._models = tuple(
                nx.lexicographical_topological_sort(
                    self.graph, key=lambda model: format_relation(model.make_relation())
                )
            )
    ...
    #     mapping = {node: format_name(node, names) for node in self.graph.nodes}
    ...
    def render_list(self) -> Table:
        select_names = get_names_from_select(self.select)
        table = Table()
        ...
        for model in self.models:
            table.add_row(
                model.__name__,
                format_relation(model.make_relation()),
                format_type(model),
...
class Runner(BaseModel):
    ...
    def run(self) -> None:
        ...
        models = pydash.filter_(
            self.graph.models,
            lambda model: (
                get_model_schema(model) != DW_SCHEMA
                and model.__materialization__ != Materialization.EXTERNAL
            ),
        )
        ModelRun: ModelType = pydash.find(
            self.graph.models,
            lambda model: get_model_schema(model) == DW_SCHEMA and model.__name__ == "ModelRun",
        )
        invocation_id = uuid.uuid4()
        log.info(f"Started run {invocation_id}")
        ...
        with timed() as invocation_timing, self.adapter.create_session() as session:
            for model in models:
                ...
                if model in skipped_models:
                    log.info(f"Skipped running model '{model.__name__}'")
                    statement = f"""
                ...
                log.info(f"Started running model '{model.__name__}'")
                statement = f"""
                ...
                with timed() as model_run_timing:
                    try:
                        message = run_model(self.adapter, model)
